	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (data.equalsIgnoreCase("Unable to scan")) {
				response.sendRedirect("distQrUpload.jsp?unable");
			} else {
				if (qr_data.length < 7) {
					response.sendRedirect("distQrUpload.jsp?unable");
				} else {
					...
					if (!(id.equals((String) request.getSession().getAttribute("loginEmail")))) {
						response.sendRedirect("distQrUpload.jsp?notuser");
					} else if (qr_data[9].equals(Values.Dist)) {
						...
						String sql= "SELECT * FROM product_dist_req WHERE id = '" + qr_data[7] + "';";
						...
						if (rs.next()) {
							if (!(rs.getString(rs.findColumn("status")).equals("Shipped"))) {
								response.sendRedirect("./distQrDetails.jsp?id=" + id + "," + timestamp);							
							} else {
								response.sendRedirect("./distQrUpload.jsp?dup_qr");
							}
						} else {
							response.sendRedirect("./distQrUpload.jsp?notvalid");
						}
